    ...
    <div class="app-container"
      ...
      <!-- <div class="navi-bar">
        <div class="navi-bar-box">
            ...
            <div class="menu">
              <router-link class="link"
                ...
                <div class="menu-item">
                  ...
                  <div v-show="unreadCount > 0"
                       class="unread-text">{{ unreadCount }}</div>
                </div>
              </router-link>
              <router-link class="link"
                           v-bind:to="'/home/friend'">
                <div class="menu-item">
                  <span class="icon iconfont icon-friend"></span>
                  <div v-show="recvCount > 0"
                       class="unread-text">{{ recvCount }}</div>
                </div>
              </router-link>
              <router-link class="link"
                           v-bind:to="'/home/group'">
                <div class="menu-item">
                  <span class="icon iconfont icon-group"
          ...
          <div class="botoom">
            <div class="botoom-item"
                 @click="isFullscreen = !isFullscreen">
              <i class="el-icon-full-screen"></i>
            </div>
            <div class="botoom-item"
                 @click="showSetting">
              <span class="icon iconfont icon-setting"
                    style="font-size: 20px"></span>
            </div>
            <div class="botoom-item hover"
                 @click="switchUser">
              <span class="icon iconfont icon-cut"
                    style="font-size: 20px"></span>
            </div>
            <div class="botoom-item"
                 @click="onExit()"
  ...
  methods: {
    init () {
      ...
      this.loadStore().then(() => {
        ...
        console.log("WebSocket连接成功", this.friendStore.friends);
        console.log("WebSocket连接成功", this.chatStore.chats);
    ...
    handlePrivateMessage (msg) {
      ...
      if (msg.type == this.$enums.MESSAGE_TYPE.FRIEND_ONLINE) {
        console.log("好友在线状态更新:", msg.content);
        this.friendStore.updateOnlineStatus(JSON.parse(msg.content));
        let content = {
          id: JSON.parse(msg.content).userId,
          online: JSON.parse(msg.content).online,
          onlineApp: JSON.parse(msg.content).terminal == 1 ? true : false,
          onlineWeb: JSON.parse(msg.content).terminal == 2 ? true : false
        }
    ...
    loadFriendInfo (id) {
      let friend = this.groupStore.findGroup(id);
      ...
      .navi-bar-box {
        ...
        .botoom {
          ...
          .icon-cut {
            width: 100%;
            height: 100%;
            background: url("../assets/icon/cut.svg") no-repeat center center;
            background-size: 50%;
          }
          .icon-cut:hover {
            background: url("../assets/icon/cut-hover.svg") no-repeat center
              center;
            background-size: 50%;
          }
        }
