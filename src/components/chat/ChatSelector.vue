  ...
  <el-dialog v-dialogDrag
    ...
    <div class="chat-selector">
      <div class="left-box">
        ...
        <tabs :items="tabItems"
              :current="tabIdx"
              @change="onTabChange"></tabs>
        <virtual-scroller v-if="tabIdx == 0"
                          class="left-scroll-box"
                          :items="showChats">
          <template v-slot="{ item }">
            <div class="chat-item"
                 @click="onClickChatItem(item)">
              <head-image :url="item.headImage"
                          :name="item.showName"
                          :size="42"></head-image>
              <div class="chat-info">
                <div class="chat-name">{{ item.showName }}
                  <el-tag v-if="item.type == 'GROUP'"
                          size="mini">群</el-tag>
                </div>
                <div class="chat-content"
                     v-html="$emo.transform(item.lastContent, 'emoji-small')">
                </div>
              </div>
        ...
        <virtual-scroller v-if="tabIdx == 1"
                          class="left-scroll-box"
                          :items="showFriends">
          <template v-slot="{ item }">
        ...
        <virtual-scroller v-if="tabIdx == 2"
                          class="left-scroll-box"
                          :items="showGroups">
          <template v-slot="{ item }">
      ...
      <div class="right-box">
        ...
        <el-scrollbar class="right-scroll-box">
          <div v-for="(item, idx) in checkedItems"
               :key="idx">
            <div class="checked-item">
              ...
              <div class="close"
                   title="移除"
                   @click.stop="onRemoveItem(item)"><i class="el-icon-close"></i>
              </div>
            </div>
          </div>
        </el-scrollbar>
