<template>
  <div class="chat-group-side">
    <chat-group-member v-if="showAllMembers"
                       @back="showAllMembers = false"
                       :group="group"
                       :groupMembers="showMembers"></chat-group-member>
    <div v-else>
      <div class="member-area">
        <div class="member-list">
          <div v-for="(member, idx) in showMembers"
               :key="member.id">
            <group-member-item v-if="idx < showMaxIdx"
                               class="group-side-member"
                               :group="group"
                               :groupMembers="groupMembers"
                               :member="member"
                               type="card"></group-member-item>
          </div>
          ...
          <div class="member-tools"
            ...
            <group-member-selector ref="removeSelector"
                                   title="选择成员进行移除"
                                   :group="group"
          ...
          <div class="member-tools"
            ...
            <div class="tool-btn"
                 title="禁言"
                 @click="onMuted()">
              <span class="icon iconfont icon-chat-muted"
                    style="font-size: 16px"></span>
            </div>
            <div class="tool-text">禁言</div>
            <group-member-selector ref="mutedSelector"
                                   title="选择成员进行禁言"
                                   :group="group"
          ...
          <div class="member-tools"
            ...
            <div class="tool-btn"
                 title="解除禁言"
                 @click="onUnmuted()">
              <span class="icon iconfont icon-chat-unmuted"
                    style="font-size: 16px"></span>
            </div>
            <div class="tool-text">解除禁言</div>
            <group-member-selector ref="unmutedSelector"
                                   title="选择成员进行解除禁言"
                                   :group="group"
        ...
        <div class="more-member"
             @click="onShowMoreMember">查看全部群成员{{ showMembers.length }}人 ></div>
      </div>
      <el-divider v-if="!group.quit"
                  content-position="center"></el-divider>
      <el-form labelPosition="top"
               class="group-side-form"
               :model="group"
               size="small">
        <el-form-item label="全员禁言"
                      title="开启全员禁言后，仅群主可以发言">
          <el-switch :disabled="(!isOwner && !isManager) || !editing"
                     v-model="group.isMuted"> </el-switch>
        </el-form-item>
        <el-form-item label="群聊名称">
          <el-input v-model="group.name"
                    :disabled="(!isOwner && !isManager) || !editing"
                    maxlength="20"></el-input>
        </el-form-item>
        <el-form-item label="群主">
          <el-input :value="ownerName"
                    disabled></el-input>
        </el-form-item>
        <el-form-item label="群公告">
          <el-input v-model="group.notice"
                    :disabled="(!isOwner && !isManager) || !editing"
                    type="textarea"
                    maxlength="1024"></el-input>
        </el-form-item>
        <el-form-item label="备注">
          <el-input v-model="group.remarkGroupName"
                    :disabled="!editing"
                    maxlength="20"></el-input>
        </el-form-item>
        <el-form-item label="我在本群的昵称">
          <el-input v-model="group.remarkNickName"
                    :disabled="!editing"
                    maxlength="20"></el-input>
        </el-form-item>
        <div v-show="!group.quit"
             class="btn-group">
          <el-button v-if="editing"
                     type="success"
                     @click="onSaveGroup()">保存</el-button>
          <el-button v-if="!editing"
                     type="primary"
                     @click="editing = !editing">编辑</el-button>
          <el-button type="danger"
                     v-show="!isOwner"
                     @click="onQuit()">退出群聊</el-button>
        </div>
      </el-form>
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    showMaxIdx () {
      console.log(this.isOwner || this.isManager ? 11 : 14)
