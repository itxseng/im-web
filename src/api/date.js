let toTimeText = (timeStamp, simple, text) => {
  ...
  if (timeDiff <= 60000) { //一分钟内
    ...
    //今日
    timeText = '今天' + formatDateTime(dateTime).substr(11, 5) + (text ? '之前在线' : '');
  } else if (isYestday(dateTime)) {
    //昨天
    timeText = '昨天' + formatDateTime(dateTime).substr(11, 5) + (text ? '之前在线' : '');
  } else if (isYear(dateTime)) {
    //今年
    timeText = formatDateTime(dateTime).substr(5, simple ? 5 : 14) + (text ? '之前在线' : '');
  } else {
    //不属于今年
